...
public class TiledMapUtils{
    ...
    public static boolean setTile(final TiledMap map,OrthographicCamera cam,int l,int x,int y,int id){
        if(map.getTileId(l,x,y)!=id && map.inBounds(l,x,y)){
            if(id==6 || id==8)
                l=1;
            int t=map.getTileId(l,x,y);
            if(t==4 || t==140){
                Lights.removeLight(x,y);
            }
            ...
            if(map.getTileId(l,x,y-1)!=5 && map.getTileId(l,x,y-1)!=7)
                map.setTileData(l,x,y-1,TiledMapUtils.data(map,l,x,y-1));
            if(id==4)
                Lights.addLight(x,y,6,22,1,1,1);
            else if(id==140)
                Lights.addLight(x,y,6,22,0.1f,0.2f,1f);
