...
public class GdxMatrix4 {
    ...
    static final GdxMatrix4 tmpMat=new GdxMatrix4();
    ...
    public GdxMatrix4(GdxMatrix4 matrix){
        set(matrix);
    }

    public GdxMatrix4(float[] values){
        set(values);
    }

    public GdxMatrix4(Quaternion quaternion){
        set(quaternion);
    }

    public GdxMatrix4(Vector3 position, Quaternion rotation, Vector3 scale){
        set(position,rotation,scale);
    }

    public GdxMatrix4 set(GdxMatrix4 matrix){
        return set(matrix.val);
    }

    public GdxMatrix4 set(float[] values){
        System.arraycopy(values,0,val,0,val.length);
        return this;
    }

    public GdxMatrix4 set(Quaternion quaternion){
        return set(quaternion.x,quaternion.y,quaternion.z,quaternion.w);
    }

    public GdxMatrix4 set(float quaternionX, float quaternionY, float quaternionZ, float quaternionW){
        return set(0f,0f,0f,quaternionX,quaternionY,quaternionZ,quaternionW);
    }

    public GdxMatrix4 set(Vector3 position, Quaternion orientation){
        return set(position.x,position.y,position.z,orientation.x,orientation.y,orientation.z,orientation.w);
    }

    public GdxMatrix4 set(float translationX, float translationY, float translationZ, float quaternionX, float quaternionY, float quaternionZ, float quaternionW){
    ...
    public GdxMatrix4 set(Vector3 position, Quaternion orientation, Vector3 scale){
    ...
    public GdxMatrix4 set(float translationX, float translationY, float translationZ, float quaternionX, float quaternionY, float quaternionZ, float quaternionW, float scaleX, float scaleY, float scaleZ){
        final float xs=quaternionX*2f, ys=quaternionY*2f, zs=quaternionZ*2f;
        final float wx=quaternionW*xs, wy=quaternionW*ys, wz=quaternionW*zs;
        final float xx=quaternionX*xs, xy=quaternionX*ys, xz=quaternionX*zs;
        final float yy=quaternionY*ys, yz=quaternionY*zs, zz=quaternionZ*zs;
    ...
    public GdxMatrix4 set(Vector3 xAxis, Vector3 yAxis, Vector3 zAxis, Vector3 pos){
        val[M00]=xAxis.x;
        val[M01]=xAxis.y;
        val[M02]=xAxis.z;
        val[M10]=yAxis.x;
    ...
    public GdxMatrix4 cpy(){
        return new GdxMatrix4(this);
    }

    public GdxMatrix4 trn(Vector3 vector){
        val[M03]+=vector.x;
        val[M13]+=vector.y;
        val[M23]+=vector.z;
        return this;
    ...
    public GdxMatrix4 trn(float x, float y, float z){
        val[M03]+=x;
        val[M13]+=y;
        val[M23]+=z;
        return this;
    ...
    public GdxMatrix4 mul(GdxMatrix4 matrix){
        mul(val,matrix.val);
        return this;
    }

    public GdxMatrix4 mulLeft(GdxMatrix4 matrix){
        tmpMat.set(matrix);
        mul(tmpMat.val,val);
        return set(tmpMat);
    }

    public GdxMatrix4 tra(){
        float m01=val[M01];
        float m02=val[M02];
        float m03=val[M03];
        float m12=val[M12];
    ...
    public GdxMatrix4 rotationX(float ang){
        float cos=(float)Math.cos(ang*Maths.toDegrees);
        float sin=(float)Math.sin(ang*Maths.toDegrees);
        val[M00]=1.0f;
        val[M01]=0.0f;
        val[M02]=0.0f;
        val[M03]=0.0f;
    ...
    public GdxMatrix4 rotationY(float ang){
        float cos=(float)Math.cos(ang*Maths.toDegrees);
        float sin=(float)Math.sin(ang*Maths.toDegrees);
        val[M00]=cos;
        val[M01]=0.0f;
    ...
    public GdxMatrix4 rotationZ(float ang){
        float cos=(float)Math.cos(ang*Maths.toDegrees);
        float sin=(float)Math.sin(ang*Maths.toDegrees);
        val[M00]=cos;
        val[M01]=sin;
    ...
    public GdxMatrix4 setToProjection(float width, float height, float near, float far, float fovy){
        idt();

        float y_scale=1/(float)Math.tan(fovy*Maths.toRadians/2);
        float frustum_length=near-far;

        val[M00]=height/width*y_scale;
        val[M10]=0;
        val[M20]=0;
        val[M30]=0;
        val[M01]=0;
        ...
        val[M02]=0;
        val[M12]=0;
        val[M22]=(near+far)/frustum_length;
        val[M32]=(near*far*2)/frustum_length;
        val[M03]=0;
        val[M13]=0;
        val[M23]=-1;
        val[M33]=0;
    ...
    public GdxMatrix4 setToProjection(float left, float right, float bottom, float top, float near, float far){
    ...
    public GdxMatrix4 setToOrtho2D(float x, float y, float width, float height){
        setToOrtho(x,x+width,y,y+height,0,1);
        return this;
    }

    public GdxMatrix4 setToOrtho2D(float x, float y, float width, float height, float near, float far){
        setToOrtho(x,x+width,y,y+height,near,far);
        return this;
    }

    public GdxMatrix4 setToOrtho(float left, float right, float bottom, float top, float near, float far){
    ...
    public GdxMatrix4 setTranslation(Vector3 vector){
        val[M03]=vector.x;
        val[M13]=vector.y;
        val[M23]=vector.z;
        return this;
    ...
    public GdxMatrix4 setTranslation(float x, float y, float z){
        val[M03]=x;
        val[M13]=y;
        val[M23]=z;
        return this;
    ...
    public GdxMatrix4 setToTranslation(Vector3 vector){
        idt();
        val[M03]=vector.x;
        val[M13]=vector.y;
        val[M23]=vector.z;
    ...
    public GdxMatrix4 setToTranslation(float x, float y, float z){
        idt();
        val[M03]=x;
        val[M13]=y;
        val[M23]=z;
    ...
    public GdxMatrix4 setToTranslationAndScaling(Vector3 translation, Vector3 scaling){
        idt();
    ...
    public GdxMatrix4 setToTranslationAndScaling(float translationX, float translationY, float translationZ, float scalingX, float scalingY, float scalingZ){
        idt();
        val[M03]=translationX;
        val[M13]=translationY;
        val[M23]=translationZ;
    ...
    public GdxMatrix4 setToRotation(Vector3 axis, float degrees){
        if(degrees==0){
            idt();
            return this;
        }
    ...
    public GdxMatrix4 setToRotationRad(Vector3 axis, float radians){
        if(radians==0){
            idt();
            return this;
        }
    ...
    public GdxMatrix4 setToRotation(float axisX, float axisY, float axisZ, float degrees){
        if(degrees==0){
            idt();
            return this;
        }
    ...
    public GdxMatrix4 setToRotationRad(float axisX, float axisY, float axisZ, float radians){
        if(radians==0){
            idt();
            return this;
        }
    ...
    public GdxMatrix4 setToRotation(final Vector3 v1, final Vector3 v2){
        return set(quat.setFromCross(v1,v2));
    }

    public GdxMatrix4 setToRotation(final float x1, final float y1, final float z1, final float x2, final float y2, final float z2){
        return set(quat.setFromCross(x1,y1,z1,x2,y2,z2));
    }

    public GdxMatrix4 setFromEulerAngles(float yaw, float pitch, float roll){
        quat.setEulerAngles(yaw,pitch,roll);
        return set(quat);
    }

    public GdxMatrix4 setFromEulerAnglesRad(float yaw, float pitch, float roll){
        quat.setEulerAnglesRad(yaw,pitch,roll);
        return set(quat);
    }

    public GdxMatrix4 setToScaling(Vector3 vector){
        idt();
        val[M00]=vector.x;
        val[M11]=vector.y;
        val[M22]=vector.z;
    ...
    public GdxMatrix4 setToScaling(float x, float y, float z){
        idt();
        val[M00]=x;
        val[M11]=y;
        val[M22]=z;
    ...
    public GdxMatrix4 setToLookAt(Vector3 direction, Vector3 up){
    ...
    public GdxMatrix4 setToLookAt(Vector3 position, Vector3 target, Vector3 up){
    ...
    public GdxMatrix4 setToWorld(Vector3 position, Vector3 forward, Vector3 up){
        tmpForward.set(forward).nor();
        right.set(tmpForward).crs(up).nor();
        tmpUp.set(right).crs(tmpForward).nor();
        set(right,tmpUp,tmpForward.mul(-1),position);
        return this;
    }

    public GdxMatrix4 lerp(GdxMatrix4 matrix, float alpha){
        for(int i=0; i<16; i++)
            val[i]=val[i]*(1-alpha)+matrix.val[i]*alpha;
        return this;
    }

    public GdxMatrix4 set(Matrix3 mat){
        val[0]=mat.val[0];
        val[1]=mat.val[1];
        val[2]=mat.val[2];
        val[3]=0;
    ...
    public GdxMatrix4 set(Affine2 affine){
        val[M00]=affine.m00;
        val[M10]=affine.m10;
        val[M20]=0;
        val[M30]=0;
    ...
    public GdxMatrix4 setAsAffine(Affine2 affine){
        val[M00]=affine.m00;
        val[M10]=affine.m10;
        val[M01]=affine.m01;
        val[M11]=affine.m11;
    ...
    public GdxMatrix4 setAsAffine(GdxMatrix4 mat){
        val[M00]=mat.val[M00];
        val[M10]=mat.val[M10];
        val[M01]=mat.val[M01];
        val[M11]=mat.val[M11];
    ...
    public GdxMatrix4 scl(Vector3 scale){
        val[M00]*=scale.x;
        val[M11]*=scale.y;
        val[M22]*=scale.z;
        return this;
    ...
    public GdxMatrix4 scl(float x, float y, float z){
        val[M00]*=x;
        val[M11]*=y;
        val[M22]*=z;
        return this;
    ...
    public GdxMatrix4 scl(float scale){
        val[M00]*=scale;
        val[M11]*=scale;
        val[M22]*=scale;
        return this;
    ...
    public float getScaleX(){
        return (Maths.isZero(val[M01]) && Maths.isZero(val[M02])) ? Math.abs(val[M00]) : (float)Math.sqrt(getScaleXSquared());
    }

    public float getScaleY(){
        return (Maths.isZero(val[M10]) && Maths.isZero(val[M12])) ? Math.abs(val[M11]) : (float)Math.sqrt(getScaleYSquared());
    }

    public float getScaleZ(){
        return (Maths.isZero(val[M20]) && Maths.isZero(val[M21])) ? Math.abs(val[M22]) : (float)Math.sqrt(getScaleZSquared());
    }
    ...
    public GdxMatrix4 toNormalMatrix(){
        val[M03]=0;
        val[M13]=0;
        val[M23]=0;
        return inv().tra();
	...
	*/
    ...
    public GdxMatrix4 translate(Vector3 translation){
        return translate(translation.x,translation.y,translation.z);
    }

    public GdxMatrix4 translate(float x, float y, float z){
    ...
    public GdxMatrix4 rotate(Vector3 axis, float degrees){
        if(degrees==0)
            return this;
    ...
    public GdxMatrix4 rotateRad(Vector3 axis, float radians){
        if(radians==0)
            return this;
    ...
    public GdxMatrix4 rotate(float axisX, float axisY, float axisZ, float degrees){
        if(degrees==0)
            return this;
    ...
    public GdxMatrix4 rotateRad(float axisX, float axisY, float axisZ, float radians){
        if(radians==0)
            return this;
    ...
    public GdxMatrix4 rotate(Quaternion rotation){
    ...
    public GdxMatrix4 rotate(final Vector3 v1, final Vector3 v2){
        return rotate(quat.setFromCross(v1,v2));
    }

    public GdxMatrix4 rotateTowardDirection(final Vector3 direction, final Vector3 up){
        l_vez.set(direction).nor();
        l_vex.set(direction).crs(up).nor();
        l_vey.set(l_vex).crs(l_vez).nor();
    ...
    public GdxMatrix4 rotateTowardTarget(final Vector3 target, final Vector3 up){
        tmpVec.set(target.x-val[M03],target.y-val[M13],target.z-val[M23]);
        return rotateTowardDirection(tmpVec,up);
    }

    public GdxMatrix4 scale(float scaleX, float scaleY, float scaleZ){
        val[M00]*=scaleX;
        val[M01]*=scaleY;
        val[M02]*=scaleZ;
        val[M10]*=scaleX;
    ...
    public boolean hasRotationOrScaling(){
        return !(Maths.isEqual(val[M00],1) && Maths.isEqual(val[M11],1) && Maths.isEqual(val[M22],1) && Maths.isZero(val[M01]) && Maths.isZero(val[M02]) && Maths.isZero(val[M10]) && Maths.isZero(val[M12]) && Maths.isZero(val[M20]) && Maths.isZero(val[M21]));
    }
