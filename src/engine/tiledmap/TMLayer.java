...
public class TMLayer implements Serializable{
	...
	public void draw(TileManager tileManager,OldSpriteBatch sb,OrthographicCamera cam){
		if(show){
	...
	public void draw(TileManager tileManager,OldSpriteBatch sb,OrthographicCamera cam,float px,float py){
		if(show){
			Vector2i start=getStartRender(cam);
			Vector2i end=getEndRender(cam);
			for(int tx=start.x; tx<end.x; tx++){
